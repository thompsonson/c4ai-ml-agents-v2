...
class TestEvaluation:
    ...
    def test_evaluation_complete_with_results(self) -> None:
        """Test evaluation can transition from running to completed with results."""
        evaluation_id = uuid.uuid4()
        benchmark_id = uuid.uuid4()
        agent_config = AgentConfig(
            agent_type="none",
        ...
        completed_evaluation = evaluation.complete_with_results(results)
        ...
        assert completed_evaluation.results == results
    ...
    def test_evaluation_complete_with_results_invalid_state(self) -> None:
        """Test complete_with_results fails when not in running state."""
        evaluation = self._create_test_evaluation(status="pending")
    ...
    def test_evaluation_validation_completed_requires_results(self) -> None:
        """Test completed evaluation must have results."""
        evaluation_id = uuid.uuid4()
        benchmark_id = uuid.uuid4()
        agent_config = AgentConfig(
            agent_type="none",
        ...
        try:
            Evaluation(
                ...
                results=None,  # Missing results
                failure_reason=None,
            )
            raise AssertionError("Expected ValueError for missing results")
        except ValueError as e:
            assert "Completed evaluation must have results" in str(e)
