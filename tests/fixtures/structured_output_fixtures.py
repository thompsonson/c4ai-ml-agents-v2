...
def mock_parsed_response_factory():
    ...
    def create_response(content: str, structured_data: dict[str, Any] | None = None,
                       token_usage: dict[str, int] | None = None) -> ParsedResponse:
        """Create a mock ParsedResponse with specified data."""
        response = Mock(spec=ParsedResponse)
        response.content = content
        response.structured_data = structured_data
        response.token_usage = token_usage or {"prompt_tokens": 10, "completion_tokens": 5, "total_tokens": 15}
